...
>
> 说明：本文是总体架构总览，负责描述目标架构与阶段划分。`Phase 0-3` 的冻结实施方案、基线校正项与接口边界，统一维护在 [`frozen-plan/`](frozen-plan/README.md)，并以该目录中的文档作为后续阶段执行输入。
...
因此本文中的部分内容是目标架构描述，而不是“当前代码已实现能力”说明。针对新的 `Phase 1-3` 实现边界，本轮冻结结论如下：

- `runtime-kernel` 只负责 session / turn 生命周期、状态校验、事件追加与审批状态
- `runtime-agent` 负责完整推理循环、PromptManager、tool loop 与消息工作轨
- `runtime-host` 继续作为唯一 composition root，并在新 `Phase 3` 内承载 Protocol Handler
- WebSocket / stdio transport 后置到新 `Phase 4`
...
runtime-agent ← 【新增】推理循环、PromptManager、CompactionStrategy、Sub-agent
  ↑
runtime-host ← 组装层（注入所有实现、Phase 3 承载 Protocol Handler，Phase 4 接入传输层）
  ↑
...
| `runtime-host` | 扩展 | 注入 runtime-agent、在 Phase 3 承载 Protocol Handler、在 Phase 4 接入 transport |
...
- 新 `Phase 3` 内新增 host-owned Protocol Handler 边界
- 新 `Phase 4` 再新增传输层抽象（见第 6 节）
- `RuntimeHostConfig` 扩展 agent 相关配置（配置包路径、compaction 策略等）

## 6. 传输层设计

> 本节描述的是目标态 transport 架构。按冻结后的实施边界，`Protocol Handler` 在新 `Phase 3` 先落在 `runtime-host` 内，WebSocket / stdio transport 的具体实现统一后置到新 `Phase 4`。
...
**目标**：统一职责、术语与阶段映射，作为后续实现前置门。

### Phase 1：最小可运行 Agent MVP

**目标**：本地/in-process 跑通 LLM API、最小 loop、tool call、MCP 工具使用，并提供 `chrome-devtools` 示例场景。

| 任务 | Crate | 说明 |
|------|-------|------|
| 创建 runtime-agent crate 最小入口 | runtime-agent | 本地/in-process agent MVP |
| 实现最小 tool call 与 MCP 接线 | runtime-agent | LLM → tool → MCP → result |
| 轻量 prompt 组装 | runtime-agent | 不要求配置包 |
| 端到端验证：提示词 → 浏览器工具 → 分析输出 | 集成验证 | `chrome-devtools` 示例 |

### Phase 2：结构基础收敛

**目标**：MessageStore、最小 Prompt 配置包、runtime-agent 结构收敛。

| 任务 | Crate | 说明 |
|------|-------|------|
| 定义 Message/Part/SceneId 类型 | runtime-core | 领域模型扩展 |
| 实现 MessageStore trait + memory 后端 | runtime-store, runtime-store-memory | 工作状态存储 |
| runtime-agent foundation refactor | runtime-agent | 从临时 history 过渡到工作轨 |
| 实现基础 PromptManager（单配置包） | runtime-agent | 文件加载 + 模板解析 |

### Phase 3：完整对话能力

**目标**：approval continuation、多轮对话、sqlite MessageStore，以及 host 内部 Protocol Handler 边界。
