...
在协作应用服务端实现文件上传/下载、消息附件（图片/文件）、S3 兼容对象存储集成的后端：数据模型、存储、上传服务、REST API、数据库迁移。

> Flutter UI 部分在 Phase 2 的 U-B2.6 实现。
...
- [ ] `feat(collab): add file upload, attachment and S3 object storage backend APIs`
