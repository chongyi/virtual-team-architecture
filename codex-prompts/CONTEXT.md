...
| 数据库 | PostgreSQL 15+ | 业务数据（sqlx + SeaORM 2.0） |
| DB 迁移 | SeaORM Migration | 类型安全迁移，替代 raw SQL |
...
| 请求验证 | validator | `#[derive(Validate)]` 宏 |
| HTTP 客户端 | reqwest + reqwest-middleware | 重试、超时中间件 |
| 错误处理 | thiserror（库层）+ anyhow（边界层） | 库 API 不返回 anyhow::Error |
| 日志/追踪 | tracing | 结构化日志，非 log crate |
| 定时任务 | tokio-cron-scheduler | Phase 5+ 日程/定时器 |
...
| Flutter 图片缓存 | cached_network_image | 占位图 + 磁盘缓存 |
| Flutter 富文本 | flutter_markdown | MarkdownBody 渲染 |
...
8. 数据库访问使用 `sqlx` + `SeaORM 2.0`；迁移使用 SeaORM Migration；复杂查询可用 SeaORM Query Builder 或 sqlx 命名参数
