...
公测版最终加固：i18n 基础（中/英双语）、移动端构建验收、Phase 4-5 运维债务修复（WebSocket 并发、热池协程、认证、优雅关闭）、Phase 5 审阅缺口（Bitable 标记、Scheduler cron/recurrence、意图分类升级）。
...
- [ ] **Bitable 行 source=ve 标记**：`bitable/create_row` 在 Tool Action Gateway 中为 VE 创建的行数据添加 `source: "ve"` 标记
- [ ] **Scheduler cron 表达式修正**：将 scheduler 中 demo 级的 `0 * * * * *`（每秒）替换为实际配置的 cron 表达式，由 event/reminder 的 due_at 驱动
- [ ] **Scheduler recurrence 逻辑**：为循环事件（daily/weekly/monthly）实现正确的下次触发时间计算和自动重新调度
- [ ] **意图分类升级**：将关键词匹配分类器替换为 Cheap 模型调用（复用现有 ModelSelector），覆盖 4 种基础意图，保留关键词匹配作为 fallback
