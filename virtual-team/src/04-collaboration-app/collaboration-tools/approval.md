...
审批流是**治理型第一方扩展**。它不是普通工作产物工具，而是协作应用权限、风控和人类确认机制的可见化入口；同时它也以工具对象、审批卡片和审批记录的形式参与协作。
...
审批是 VE 权限模型的一部分——不是所有操作都需要审批，由配置包的 `permissions.toml`、Runtime Config 和 Tool Action 的 `require_approval` 声明共同决定。
