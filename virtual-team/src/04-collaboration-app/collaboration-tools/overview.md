...
协作工具是 Virtual Team 协作应用中**工作产物、工作治理和工作调度的载体**。虚拟员工的详细工作过程和大型交付内容不输出到聊天框，而是在协作文档、表格、任务/看板等工具中呈现；需要人类确认的高风险操作进入审批；需要未来触发的工作进入日程或定时器。
...
工具分组遵循成熟协作应用的用户认知，而不是底层实现复杂度。用户看到的是文档、表格、任务、审批、日程这些熟悉概念；扩展系统是内部架构和未来开放能力，不是普通用户的主概念。

### 工作产物扩展

| 工具 | 基础版定位 | 完整形态方向 | 类比 | 详细章节 |
|------|------------|--------------|------|---------|
| **文档** | 轻量 Block 文档，承载报告、方案、知识整理 | 完整协作文档、实时协同、评论、复杂嵌入 | Notion / 飞书文档 / Slack Canvas | [文档](./documents.md) |
| **多维表格** | 类型化数据表，承载结构化数据和分析结果 | 多视图、公式、关联记录、仪表盘 | Airtable / 飞书多维表格 | [多维表格](./bitable.md) |
| **任务/看板** | 卡片、列表和状态流转，承载执行进度 | 自定义工作流、跨项目视图、自动化 | Trello / Asana / Linear | [任务看板](./board.md) |

### 治理型扩展

| 工具 | 基础版定位 | 完整形态方向 | 类比 | 详细章节 |
|------|------------|--------------|------|---------|
| **审批流** | 高风险操作的人类确认，承载审批卡片和审批记录 | 多级审批、条件分支、组织级审批模板 | 飞书审批 / 企业审批流 | [审批流](./approval.md) |

### 调度型扩展

| 工具 | 基础版定位 | 完整形态方向 | 类比 | 详细章节 |
|------|------------|--------------|------|---------|
| **日程/定时器** | 时间触发、提醒和 VE 工作计划可见性 | 日历视图、重复计划、跨 VE 调度 | 日历 / 闹钟 / Reminder | [日程与定时器](./schedule-and-timer.md) |

文档和多维表格的完整形态复杂度较高，因此采用**基础版可冻结、完整形态作指导**的分层策略。基础版是开发实施口径；完整形态用于确定长期方向、Tool Surface 策略和底层开源项目参考，不作为首期实现承诺。
...
- 删除默认软删除，保留审计和引用。**各工具子文档的 DDL 中必须包含 `archived_at` 和 `deleted_at` 字段**以与统一生命周期对齐。
- 归档对象仍可被历史消息和工作上下文引用。
- 由 VE 创建或修改的对象必须记录 `created_by_type = virtual_employee` 或对应更新者，并写入审计事件。
- 与工作上下文绑定的对象，删除前需要检查是否仍是活跃任务的关键产物。若绑定活跃 work_context，应返回警告而非直接删除。
- 用户和 VE 同时修改时，必须走乐观锁冲突处理。冲突时优先保留用户修改，VE 修改进入冲突队列由 VE 重新评估。
- 工具对象与 work_context 的绑定关系默认可变更（除非该 work_context 已归档），变更需记录审计。
- 通知聚合窗口 30 秒：同一 VE 在同一 work_context 内对同一对象的多次更新聚合为一次摘要，避免刷屏。
...
| `collab.document.search` | 搜索文档 | 主 Agent / 意图 Agent |
| `collab.bitable.create` | 创建多维表格 | 主 Agent |
| `collab.bitable.add_field` | 添加字段 | 主 Agent |
| `collab.bitable.insert_rows` | 写入表格行 | 主 Agent |
| `collab.bitable.update_rows` | 更新表格行 | 主 Agent |
| `collab.bitable.query` | 查询表格 | 主 Agent |
| `collab.bitable.export` | 导出表格数据 | 主 Agent |
