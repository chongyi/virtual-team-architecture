...
flowchart TB
    ...
    subgraph trackB["轨道 B：协作应用"]
        ...
        b4 --> b5["B5: 基础版协作工具集<br/>轻量文档 + 类型化表格 + 任务/看板"]
        b5 --> b6["B6: 全平台与工具增强<br/>移动端 + 第三方 IM + 完整工具形态"]
    end
...
**B5：基础版协作工具集** ← 需要 A5 完成
- 轻量 Block 文档（基础版）
- 类型化数据表（基础版多维表格）
- 任务/看板
- 审批流（治理型扩展）
- 日程/定时器（调度型扩展）
- 虚拟员工可通过 Tool Action 在协作工具中产出内容

**B6：全平台与工具增强**
- iOS/Android 移动端适配
- 第三方 IM 渠道接入（参考 OpenClaw 的 Gateway 模式）
- 推送通知
- 完整协作文档、完整多维表格、复杂 Tool Surface 与底层开源方案评估
...
| **Phase 5** | A5（VE 封装层） | B5（基础版协作工具集） | C3（VE 绑定） | ═══ 3 线并行 |
| **Phase 6** | A6（高级特性） | B6（全平台与工具增强） | C4（高级特性） | ═══ 3 线并行 |
...
| **M5** | A5 + B5 完成 | 内测版：完整虚拟员工行为 + 基础版协作工具集 |
| **M6** | A6 + B6 + C4 完成 | 公测版：高级特性 + 全平台 + 工具增强 + 云端工作环境 |
