...
flowchart TD
    ...
    authSvr -->|签发 JWT| fe[Flutter 客户端]
    fe -->|JWT Bearer| collabSvr[协作应用服务端]

    collabSvr -->|API Key| agentSvr[Agent 服务器]
...
|------|------|
| `sub` | 用户 ID |
| `tenant_id` | 租户 ID（等于用户 ID） |
| `plan` | 付费计划（free/pro/team/enterprise） |
| `jti` | 令牌唯一 ID（用于撤销） |
