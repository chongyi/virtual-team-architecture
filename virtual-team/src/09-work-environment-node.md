...
工作环境节点（Work Environment Node）是虚拟员工执行操作的**实际物理/虚拟环境**。

传统 Agent 在本地进程内执行工具。但在 Virtual Team 中，虚拟员工运行于服务端，而用户需要其操作专业环境（本地文件、私有网络、内部系统、特定软件）。工作环境节点将"执行能力"从"推理能力"中分离出来。
