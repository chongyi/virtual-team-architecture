...
CREATE TABLE work_contexts (
    ...
    [
      ...
      { "session_id": "sess_yyy", "type": "sub", "status": "completed" }
    ]
    */
...
CREATE TABLE messages (
    ...
    -- 'text', 'rich_text', 'file', 'system', 'approval_card', 'work_summary'
    content_body TEXT,
    ...
    intent VARCHAR(32),
    -- 'new_task', 'continuation', 'simple_reply'
...
CREATE TABLE approvals (
    ...
    /*
    {
      ...
      "working_directory": "/workspace/ve_xxx/projects",
      "risk_level": "medium",
...
CREATE TABLE config_packages (
    ...
    -- 对应 manifest.toml 中的 package.name
...
| 部分索引 | 热数据（活跃/待处理状态）使用 `WHERE` 条件的部分索引减少索引体积 |
| 时序分区 | 审计日志（`audit_events`）和消息（`messages`）使用时间分区或超表 |
