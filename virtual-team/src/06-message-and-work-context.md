...
Agent Server 在使用协作应用提供的 context segment 时，可能发现数据过期或不准确。此时可通过重建 API 请求协作应用基于最新状态重新构建：

```
POST /api/v1/messages/{message_id}/context-rebuild
Authorization: Bearer <agent_server_api_key>
{
  "reason": "context_stale | markers_mismatch | manual_request",
  "requested_by": "ve_sales_01"
}
```

重建后的 context segment 由协作应用通过 WebSocket 推送给 Agent Server。重建结果不影响已持久化的消息和 markers——它只是换了一份最新的"快照"。

详见 [消息上下文增强](../04-collaboration-app/context-enhancement.md) 中的"Context Segment 的快照与重建"。

### 缓存失效策略

context segment 和 related messages 摘要受以下缓存策略约束：

- **context segment 缓存**：不缓存。每次消息转发时实时构建（构建耗时 < 50ms，不需要缓存）。
- **related messages 摘要缓存**：单条消息的摘要缓存 30 分钟。消息被编辑后缓存失效。
- **嵌入向量缓存**：同一消息的嵌入向量缓存 5 分钟（RAG 检索时使用）。

### Fork/Resume 的性能考量

| 操作 | 典型耗时 | 资源开销 | 说明 |
|------|---------|---------|------|
| Fork | < 100ms | 创建新的 WorkContext + 写入快照到 PostgreSQL | 不涉及 VTA Session 创建 |
| Resume（暂停→活跃） | < 1s | 无新 Session 创建，仅状态转换 | 已有的 VTA Session 保留在内存（温实例） |
| Resume（归档→活跃） | 3-10s | 创建新 VTA Session + 注入压缩上下文 | 需要加载摘要 + 初始化 LLM 对话 |
| Resume（含 Compaction） | 5-15s | 生成结构化摘要 + 创建新 Session | 摘要生成含一次 LLM 调用 |

Fork 操作不阻塞源工作上下文的继续执行。Fork 创建的新 WorkContext 处于 `new` 状态，等待主 Agent 开始执行后才进入 `active`。
